...
async function run(url) {
  ...
  let songIndex = readline.questionInt(
    "Enter the index of the song you want to display: "
  )
  ...
  await Promise.all([
    page.evaluate(index => {
      const titles = document.querySelectorAll(
        "div.content > div.box-przeboje > a.title"
      )
      titles[index].click()
    }, songIndex),
  ...
  contents = contents.replace("Poznaj historię zmian tego tekstu", "") // remove redundant text
  contents = contents.replace("Tekst piosenki:", "") // remove redundant text
